...
function rewriteLocalhostBaseUrl(baseUrl: string): string {
  try {
    ...
    if (!pageHost || localhostHosts.has(pageHost))
      return baseUrl
...
export function buildOpenAICompatibleProvider(
  ...
  const defaultCapabilities = {
    listModels: async (config: Record<string, unknown>) => {
      ...
      const baseUrl = rewriteLocalhostBaseUrl(normalizeBaseUrl(config.baseUrl))
  ...
  const finalValidators = validators || {
    validateProviderConfig: async (config: Record<string, unknown>) => {
      ...
      baseUrl = rewriteLocalhostBaseUrl(normalizeBaseUrl(baseUrl))
