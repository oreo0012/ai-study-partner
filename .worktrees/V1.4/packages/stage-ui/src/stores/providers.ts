...
export const useProvidersStore = defineStore('providers', () => {
  ...
  const providerMetadata: Record<string, ProviderMetadata> = {
    ...
    'volcengine': {
      ...
      validators: {
        validateProviderConfig: (config) => {
          const baseUrlString = String(config.baseUrl || '')
          const errors = [
            !config.apiKey && new Error('API key is required.'),
            !config.baseUrl && new Error('Base URL is required.'),
            !((config.app as any)?.appId) && new Error('App ID is required.'),
            baseUrlString.includes('unspeech.hyp3r.link') && new Error('The default UnSpeech SaaS endpoint requires its own grant/API key. Use your own UnSpeech instance Base URL (e.g. http://localhost:5933/v1/) to use Volcengine token/appId.'),
          ].filter(Boolean)
