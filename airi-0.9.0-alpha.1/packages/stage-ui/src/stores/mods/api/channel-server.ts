...
export const useModsServerChannelStore = defineStore('mods:channels:proj-airi:server', () => {
  ...
  const inferredProtocol = typeof window !== 'undefined' && window.location?.protocol === 'https:' ? 'wss' : 'ws'
  const inferredHost = typeof window !== 'undefined' && window.location?.hostname ? window.location.hostname : 'localhost'
  const defaultWebSocketUrl = import.meta.env.VITE_AIRI_WS_URL || `${inferredProtocol}://${inferredHost}:6121/ws`
